...
public class CashRegister implements ActionListener {
    ...
    public void scan(String action){
        String name = code.getText();
        String price = data.findPrice(name);

        if (Objects.equals(price, "PRODUCT DOES NOT EXIST")) {
            JOptionPane.showMessageDialog(frame, "PRODUCT DOES NOT EXIST");
        }

        //item is not found in the database
        else {
            if (Objects.equals(action, "+"))
            {
                String inventoryDB = product.getText() + "<br>" + name;
                String priceList = priceofProduct.getText() + "<br>+ " + price + ".00" ;
                priceofProduct.setText(priceList);
                product.setText(inventoryDB);
    ...
    // Restarts everything and goes to the start screen
    public void restart(){
    ...
    public String makeReceipt(String type){
        Set<String> distinct = new HashSet<>(receiptItems);
        String receipt = "Paid by " + type + "\n";


        for (String s: distinct) {
            String price = data.findPrice(s).substring(1);
            int qty = Collections.frequency(receiptItems, s);
